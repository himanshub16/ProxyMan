#!/bin/bash
# ProxyMan v1.1
# Author : Himanshu Shekhar < www.github.com/himanshushekharb16 >
#
# Tools required for implementation : bash, sed, grep, regex support, gsettings, apt
...
#
# This script sets the proxy configuration for a general system 
# from the Debian family.
# Configures apt, environment variables and gsettings
# The Network Manager settings (native to Gnome), sets system-wide proxy
# settings but does not set up authentication
...
toggle : Toggle current mode ( none <--> manual ) for Desktop-environment, with old configuration
...
rm apt_config.conf bash_set.conf bash_unset.conf gsettings_config.conf
echo "Job done!"
